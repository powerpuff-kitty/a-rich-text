...
Commands preserve rectangular tables, use explicit mappings for surviving cells and are undoable. Insertion accepts configurable dimensions at a single-block selection. Column operations support horizontal spans: insertion widens cells crossing the boundary; removal shrinks wide cells and removes unit cells at the active cell’s leftmost logical column. Row operations also support horizontal spans; inserted rows contain one empty cell per logical column. Removing the last row or column is a no-op. `moveTableCell` navigates in row order without adding an undo step. The standard `@arichtext/editor` controller binds Tab/Shift+Tab and permits focus to leave at table edges. Horizontal merge appends the right cell’s content blocks; split retains all content on the left and inserts empty unit cells. Tab traverses physical cells including horizontal and vertical spans, skipping rows fully occupied by earlier rowspans. Split supports horizontal, vertical and combined spans, retaining content in the top-left cell and filling the remaining rectangle with empty cells. Other spans are preserved. Vertical merging and row/column editing remain unsupported. Whole-table removal works with imported spans and leaves an editable paragraph. HTML/JSON preserve spans; Markdown/plain text do not.
