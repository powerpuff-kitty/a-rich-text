...
`@arichtext/tables` provides `insertTable`, `getActiveTable`, `addTableRow`, `removeCurrentTableRow`, `addTableColumn`, `removeCurrentTableColumn`, `removeCurrentTable`, `mergeTableCellRight`, `mergeTableCellBelow`, `splitTableCell`, `getTableCellActions`, `getTableRowActions` and `moveTableCell`. The toolbar inserts a 2 × 2 table and offers contextual row/column, merge/split and whole-table removal controls.

Commands preserve rectangular tables, use explicit mappings for surviving cells and are undoable. Insertion accepts configurable dimensions at a single-block selection. Column operations support all spans: insertion widens cells crossing the boundary; removal shrinks wide cells and removes unit cells at the active cell’s leftmost logical column. Row operations support all spans: insertion occurs outside the active cell’s full vertical extent, grows crossing spans and adds cells in uncovered columns; removal shrinks crossing spans, moves surviving originating spans into the next row and deletes unit-height cells. Removing the last row or column is a no-op. `moveTableCell` navigates in row order without adding an undo step. The standard `@arichtext/editor` controller binds Tab/Shift+Tab and permits focus to leave at table edges. Horizontal merge appends the right cell’s content blocks; split retains all content on the left and inserts empty unit cells. Tab traverses physical cells including horizontal and vertical spans, skipping rows fully occupied by earlier rowspans. Split supports horizontal, vertical and combined spans, retaining content in the top-left cell and filling the remaining rectangle with empty cells. Other spans are preserved. Merge below supports matching column boundaries and combines existing rowspans, appending lower content and mapping its anchors into the upper cell. Merging right supports adjacent cells with matching row boundaries, including rowspans, and cannot cross a carried span. Column edits account for carried rowspans; when removal leaves no physical cell in the active row, the caret moves to a surviving cell spanning that row. Whole-table removal works with imported spans and leaves an editable paragraph. HTML/JSON preserve spans; Markdown/plain text do not.
