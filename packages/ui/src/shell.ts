...
export class ARichTextShellElement extends Base {
  ...
  constructor() {
    ...
    this.attachShadow({ mode: 'open' }).innerHTML = `<style>
      ...
      ::slotted(:is(a-rich-text-toolbar, art-toolbar)) { --art-toolbar-background:#f5f7fa; --art-toolbar-radius:0; --art-toolbar-max-width:100%; --art-toolbar-padding:.5rem; border-bottom:1px solid #d5dbe5; border-radius:inherit; }
  ...
  #sync = (): void => { this.dataset.preset = this.querySelector('a-rich-text, art-editor')?.getAttribute('preset') ?? 'default'; };
}
export function defineARichTextShell(tagName = 'a-rich-text-shell'): void {
  if (typeof customElements !== 'undefined' && !customElements.get(tagName)) customElements.define(tagName, tagName === 'a-rich-text-shell' ? ARichTextShellElement : class extends ARichTextShellElement {});
}
defineARichTextShell();
defineARichTextShell('art-shell');

declare global {
  interface HTMLElementTagNameMap {
    'a-rich-text-shell': ARichTextShellElement;
    'art-shell': ARichTextShellElement;
  }
}
