...
export function importEditorJS(source: string): ConversionOutput<ARTDocument> {
  ...
  for (const block of input.blocks) {
    ...
    switch (block.type) {
      ...
      case 'list': { if (!Array.isArray(data.items) || !data.items.every(text)) throw new TypeError('Invalid Editor.js list'); const style = data.style === 'ordered' ? 'ordered' : data.style === 'unordered' ? 'bullet' : data.style === 'checklist' ? 'task' : undefined; if (!style) { notes.add('list-style', 'Unknown Editor.js list style becomes a bullet list'); } content.push({ type: 'list', style: style ?? 'bullet', content: data.items.map(item => ({ type: 'listItem', ...(style === 'task' ? { checked: false } : {}), content: [inlineHTML(item)] })) }); break; }
...
export function exportEditorJS(document: ARTDocument): ConversionOutput<string> {
  ...
  for (const block of document.content) {
    ...
    else if (block.type === 'list' && block.content.every(item => item.content.length === 1 && item.content[0]?.type === 'paragraph')) blocks.push({ type: 'list', data: { style: block.style === 'ordered' ? 'ordered' : block.style === 'task' ? 'checklist' : 'unordered', items: block.content.map(item => toHTML({ type: 'doc', version: 1, content: [item.content[0]!] }).replace(/^<p>|<\/p>$/g, '')) } });
